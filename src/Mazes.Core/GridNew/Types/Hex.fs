...
namespace Mazes.Core.GridNew.Types.Hex
...
module Grid =
    ...
    let private createInternal internalConnectionType (canvas : Canvas.Array2D.Canvas) =
...

        {
          Canvas = canvas
          Cells = cells
          PositionHandler = HexPositionHandler.Instance
          CoordinateHandler = HexCoordinateHandler.Instance
        }

    let createBaseGrid canvas =
        createInternal Close canvas :> IAdjacentStructure<GridArray2D<HexPosition>, HexPosition>

    let createEmptyBaseGrid canvas =
        createInternal Open canvas :> IAdjacentStructure<GridArray2D<HexPosition>, HexPosition>
...
module Mazes.Render.Refac.SVG.Base
...
let appendWall (sBuilder : StringBuilder) lines (connectionState : ConnectionState) coordinate =
...
let appendNormalWallBackInset (sBuilder : StringBuilder) lines (connectionState : ConnectionState) coordinate =
    match connectionState with
    | Close -> appendPathElement sBuilder None normalWallInsetBackClass lines coordinate
    | _ -> sBuilder

let appendNormalWallForeInset (sBuilder : StringBuilder) lines (connectionState : ConnectionState) coordinate =
    match connectionState with
    | Close -> appendPathElement sBuilder None normalWallInsetForeClass lines coordinate
    | _ -> sBuilder

let appendBorderWallBackInset (sBuilder : StringBuilder) lines (connectionState : ConnectionState) coordinate =
    match connectionState with
    | ClosePersistent -> appendPathElement sBuilder None borderWallInsetBackClass lines coordinate
    | _ -> sBuilder

let appendBorderWallForeInset (sBuilder : StringBuilder) lines (connectionState : ConnectionState) coordinate =
    match connectionState with
...
let appendCellColorWithDynamicOpacity lines distanceFromRoot maxDistance coordinate (sBuilder : StringBuilder) =
    let opacity = round (1.0 - (float (Distance.value (maxDistance - distanceFromRoot)) / float (Distance.value maxDistance)))
...
let appendMazeDistanceColoration map wholeCellLines (sBuilder : StringBuilder) =
    ...
    | Some map ->
        let distanceFromRoot coordinate =
            match (ShortestPathGraph.nodeDistanceFromRoot map.ShortestPathGraph coordinate) with
            | Some distance when distance = Distance 0 -> Distance 0
            | Some distance -> distance - Distance 1
            | None -> Distance 0
        ShortestPathGraph.nodes map.ShortestPathGraph
        |> Seq.iter(fun coordinate -> sBuilder |> appendCellColorWithDynamicOpacity (wholeCellLines coordinate) (distanceFromRoot coordinate) (map.FarthestFromRoot.Distance - Distance 1) (Some coordinate) |> ignore)
    | None -> ()
...
let appendMazeDistanceBridgeColoration sequence map wholeBridgeLines (sBuilder : StringBuilder) =
    ...
    | Some map ->
        ...
        |> Seq.iter(fun (fromCoordinate, toCoordinate, wallType) ->
            ...
            | Open, _ | Close, true ->
                sBuilder |> appendCellColorWithDynamicOpacity (wholeBridgeLines fromCoordinate toCoordinate) (distanceFromRoot fromCoordinate) (map.FarthestFromRoot.Distance - Distance 1) None |> ignore
            | _ -> ())
    | None -> ()
...
let appendSimpleBridges calculatePointsBridge (bridges : (NCoordinate * NCoordinate * ConnectionState) seq) (sBuilder : StringBuilder) =
    bridges
    |> Seq.iter(fun (fromCoordinate, toCoordinate, wallType) ->
            match wallType, showClosedBridge with
            | Open, _ | Close, true ->
...
let appendSimpleWallsBridges calculatePointsBridge (bridges : (NCoordinate * NCoordinate * ConnectionState) seq) (sBuilder : StringBuilder) =
    bridges
    |> Seq.iter(fun (fromCoordinate, toCoordinate, wallType) ->
